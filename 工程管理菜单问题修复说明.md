...
### 2. 更新 sys_project.sql

将 `sys_project.sql` 改为使用固定ID，并添加了说明注释：
- 如果已执行主SQL文件，不需要再执行菜单部分
- 如果只想单独添加工程管理模块，可以执行此文件

### 3. 创建更新脚本

创建了 `update_project_menu.sql` 脚本，用于更新现有数据库：
- 删除旧的动态ID菜单
- 插入固定ID的新菜单
- 更新角色菜单关联
...
# 5. 导入项目版本管理表
mysql -u root -p ruoyi-fastapi < ruoyi-fastapi-backend/sql/sys_project_version.sql
```
...
- `ruoyi-fastapi-backend/sql/ruoyi-fastapi.sql` - 主SQL文件（已更新）
- `ruoyi-fastapi-backend/sql/sys_project.sql` - 工程管理模块SQL（已更新）
- `ruoyi-fastapi-backend/sql/update_project_menu.sql` - 菜单更新脚本（新建）
- `ruoyi-fastapi-backend/sql/check_project_menu.sql` - 菜单检查脚本（新建）
...
4. 执行检查脚本查看菜单数据：
   ```bash
   mysql -u root -p ruoyi-fastapi < ruoyi-fastapi-backend/sql/check_project_menu.sql
   ```
