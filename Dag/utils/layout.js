...
import { DAG_LAYOUT_CONFIG } from '../config/layoutConfig.js'

// 简单的 dagre 风格布局实现，避免 @antv/layout 的兼容性问题
// direction: 'TB' | 'LR' (只支持竖向和横向)
export async function dagreLayout(
  graph, 
  direction = DAG_LAYOUT_CONFIG.defaultDirection, 
  ranksep = DAG_LAYOUT_CONFIG.ranksep, 
  nodesep = DAG_LAYOUT_CONFIG.nodesep
) {
  if (!graph) return
