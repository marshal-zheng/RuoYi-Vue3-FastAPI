...
import { generateNodePorts } from '../config/portConfig.js'
...
const convertToDAGFormat = (data) => {
  ...
  const edges = data.edges.map((edge) => {
    ...
    const weightEditable = edge.properties?.weightEditable !== false
    
    const edgeConfig = {
      id: edge.id,
      shape: DAG_EDGE,
      source: { cell: edge.sourceNodeId, port: 'b' },
      target: { cell: edge.targetNodeId, port: 't' },
      animated: false,
      data: {
        weight,
        weightEditable,
...
const waitForGraphAndLayout = async (nodes) => {
  ...
  if (g) {
    ...
    if (props.autoLayout && nodes.length > 0) {
      // 先让节点渲染完成，再进行布局
      await wait(200)
      await dagreLayout(g, props.layoutDirection, 220, 100)
      refreshCollapseState(g)
      g.centerContent()
    }
  }
...
watch(
  ...
  async (newDirection, oldDirection) => {
    ...
    if (!g) return
    try {
      await dagreLayout(g, newDirection, 220, 100)
      refreshCollapseState(g)
      g.centerContent?.()
    } catch (error) {}
  }
