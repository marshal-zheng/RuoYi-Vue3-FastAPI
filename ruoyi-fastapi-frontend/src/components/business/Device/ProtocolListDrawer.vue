<template>
  ...
  >
    <!-- 空状态 -->
    <div v-if="!devicePorts || devicePorts.length === 0" class="flex flex-col items-center justify-center px-5 py-16 text-center">
    ...
    >
      <!-- 总线类型分组头部 -->
      <div class="flex items-center px-4 py-3 bg-primary-50 cursor-pointer select-none transition-all border-b border-gray-200 hover:bg-primary-100" @click="togglePort(group.type)">
        <el-icon 
      ...
      <div v-show="!collapsedPorts[group.type]" class="bg-gray-50">
        <!-- 无协议提示 -->
        <div v-if="group.totalProtocols === 0" class="flex items-center justify-center gap-2 p-6 text-gray-400 text-sm">
        ...
        >
          <div class="flex items-center justify-center w-9 h-9 rounded-md bg-gradient-to-br from-primary-50 to-indigo-50 text-primary-500 text-lg shrink-0">
          ...
          <div class="flex-1 ml-3 min-w-0">
            ...
            <div class="flex items-center gap-3 flex-wrap">
              <span v-if="protocol.message.header?.sender" class="text-xs text-gray-600 whitespace-nowrap">
                发送方: {{ protocol.message.header.sender }}
              </span>
              <span v-if="protocol.message.header?.receiver" class="text-xs text-gray-600 whitespace-nowrap">
          ...
          <el-icon class="text-sm text-gray-400 shrink-0 transition-transform group-hover:translate-x-0.5 group-hover:text-primary-500"><ArrowRight /></el-icon>
        </div>
      </div>
    </div>
  </ZxDrawer>
