...
const syncConfigTableRows = () => {
  ...
  rows[1].value2 = localConfig.subAddress;
  rows[1].value3 = `${localConfig.frameLength} word`;
...
const configEditRender = (field: string) => {
  ...
  const handleUpdate = (cellParams: any, evtOrValue: any) => {
    ...
    if (field === 'value1') {
      ...
      if (row.label3 === '帧长度') {
        localConfig.frameLength = stripUnit(nextValue, 'word');
      } else if (row.label3 === '传输频率') {
        localConfig.frequency = nextValue;
      }
    } else if (field === 'value4') {
      if (row.label4 === '错误处理') {
        localConfig.errorHandle = nextValue;
      } else if (row.label4 === '传输速率') {
