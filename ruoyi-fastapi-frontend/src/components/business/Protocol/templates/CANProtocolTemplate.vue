    ...
    >
      ...
      <vxe-column
        ...
        :edit-render="{
          ...
          props: {
            dictType: 'sys_protocol_data_type',
            placeholder: '请输入或选择',
            uppercaseLabel: true,
          },
        }"
...
function getDataTypeTagType(dataType: string) {
...
const syncConfigTableRows = () => {
  ...
  rows[3].value2 = localConfig.canId;
...
const configEditRender = (field: string) => {
  const getDictType = (row: Record<string, string>) => {
    if (field === 'value1' && row.label1 === '传输方式') return 'sys_protocol_method_can';
    if (field === 'value1' && row.label1 === '传输频率') return 'sys_protocol_frequency';
    if (field === 'value2' && row.label2 === '传输速率/bps') return 'sys_protocol_speed_can';
