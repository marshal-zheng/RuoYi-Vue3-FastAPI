<template>
  <div
    class="device-port-node relative overflow-visible"
  ...
  >
    ...
    <div
      ...
      :style="{
        ...
        background:
          'linear-gradient(135deg, #434343 0%, #282828 25%, #1a1a1a 50%, #282828 75%, #434343 100%)',
        border: '1px solid rgba(100, 100, 100, 0.6)',
        boxShadow:
          '0 4px 20px rgba(0, 0, 0, 0.5), inset 0 1px 0 rgba(255, 255, 255, 0.1), inset 0 -1px 0 rgba(0, 0, 0, 0.5)',
      }"
    >
      <!-- 顶部高光条 -->
      <div
      ...
      >
        <!-- 螺丝孔中心点 -->
        <div
          class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-0.5 h-0.5 bg-black/60 rounded-full"
        ></div>
      </div>
...
const containerWidth = computed(() => {
  ...
  return (
    (hasLeftPorts ? SIDE_PORT_WIDTH : 0) + DEVICE_WIDTH + (hasRightPorts ? SIDE_PORT_WIDTH : 0)
  );
});
...
const containerHeight = computed(() => {
  ...
  return (
    (hasTopPorts ? TOP_BOTTOM_PORT_HEIGHT + TOP_BOTTOM_MARGIN : 0) +
...
const syncNodeSize = () => {
  if (
    !props.node ||
    typeof props.node.getSize !== 'function' ||
    typeof props.node.resize !== 'function'
  ) {
    return;
  }
